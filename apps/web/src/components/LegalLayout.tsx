...
export function LegalLayout({
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      <h1 className="mb-6 text-3xl font-bold text-ink-900">{title}</h1>
      {/* space-y-4 da separación uniforme entre párrafos y secciones sin tener
          que ponerla en cada elemento. */}
      <div className="space-y-4 leading-relaxed text-ink-700">{children}</div>
      <p className="mt-10 border-t border-ink-200 pt-4 text-sm text-ink-400">
...
export function LegalSection({
  ...
  return (
    <section className="space-y-2">
      <h2 className="pt-4 text-xl font-semibold text-ink-900">{title}</h2>
      {children}
    </section>
  )
}
